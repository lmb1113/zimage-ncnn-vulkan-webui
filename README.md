...
| 局部重绘 | `-i -k` | 上传原图后**在页面内直接用画笔涂抹重绘区域**（画笔/橡皮/笔刷大小/撤销/重做/清空，滚轮缩放、移动平移，`[` `]` 调笔刷），蒙版自动按原图分辨率生成并上传，无需单独上传遮罩图；也支持拖入现成遮罩。**任意尺寸原图自动适配**：服务端自动把输入图与蒙版缩放到最近的 16 倍数（引擎硬性要求），无需手动处理 |
...
- **实时进度**：解析引擎 `step n/N` 输出精确换算进度百分比，控制台同步滚动引擎日志；
  浏览器标签页标题实时显示「生成中 N%」，页面在后台时任务结束弹**系统通知**
- **任务队列**：同一时间只跑一个任务（独占显卡），后续任务自动排队，可一键清空排队，可随时取消
- **批量生成**：`-b` 批量出多张，进度按张累计
- **提示词历史**：正向 / 反向提示词均可一键回填图库中出现过的内容（去重、最新在前）
- **对比滑块**：重绘 / 图生图 / 控制生成完成后，拖动分割线对比原图与结果
- **快捷键**：`Ctrl + V` 粘贴图片、`Ctrl + Enter` 提交生成、`Ctrl + Z` 撤销笔迹、`[` `]` 调笔刷、
  `←` `→` 灯箱切图、`Esc` 关闭
- **上传**：图片输入框支持点击选择、拖拽与 `Ctrl + V` 粘贴
...
- 修复关键回归：局部重绘拖动时笔迹不显示（坐标变换系数方向错误，
  笔迹被画到画布外；现落笔/拖动/抬笔全程实时渲染，像素级验证通过）

### v0.3.7

- 全模式尺寸兜底：引擎要求宽高为 16 的倍数，文生图/图片放大的自由输入尺寸
  现在也会自动对齐到最近 16 倍数（此前仅重绘/扩图/控制生成有前置处理）

### v0.3.6

- 图片放大（Tile）模式完成端到端验证（256→1024 四倍放大实测成功）；
  修复 API 提交 tile 模式时 `-t` 标志丢失的问题
- 六种模式全部完成真实引擎端到端验证
...
- 修复局部重绘对任意尺寸原图的兼容：引擎要求 `-s` 与输入一致且宽高为 16 的倍数，
  服务端现在自动把输入图与蒙版缩放到最近的 16 倍数（输入 CatmullRom、遮罩最近邻），
  控制台显示 `[准备] 输入图 WxH 已自动缩放为 WxH` 日志
- 修复蒙版画布增量绘制的光标圈残影；长笔画会话性能恒定
- 失败任务一键重试；灯箱删除按钮；设置默认设备生效修复
...
- 蒙版编辑器：笔刷光标预览圈（实时显示涂抹范围）、重做（Ctrl+Shift+Z / Ctrl+Y）
- 蒙版画布增量绘制，长笔画会话性能恒定
- 图库「复制提示词」按钮

### v0.3.0

- 蒙版编辑器大升级：滚轮缩放（100%~800%，以光标为中心）、「移动」工具平移、适应复位；笔迹锚定图片原始坐标，任何视图变化都不漂移
- 提示词历史（正向/反向，一键回填）、后台系统通知、标签页标题实时进度、favicon
- 预览区原图/结果对比滑块；灯箱全屏对比
- 图库 ZIP 打包下载、分页渲染、不可变缓存头、删除二次确认、模型建议列表
- API 接口层测试（httptest）；修复取消任务 API 误报、输入法组词误触提交

### v0.2.5

- 图库 ZIP 打包下载（跟随当前筛选）；媒体文件不可变缓存头；详情缩略图
- 灯箱全屏对比原图/结果；删除图片二次确认防误删

### v0.2.4

- 提示词历史（正向/反向）、后台系统通知、标签页标题实时进度、favicon
- 预览区原图/结果对比滑块；图库分页与破图守卫；模型建议列表
- 界面移除命令行参数提示；修复取消任务 API 误报；步数/种子悬停语义提示
